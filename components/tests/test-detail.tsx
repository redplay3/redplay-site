  ...
  return <main className={styles.page}>
    ...
    <div className={styles.detailLayout}>
      ...
      <article className={styles.detailBody}>
        ...
        <section className={styles.videoPanel}><div className={styles.videoIcon}><Film size={28}/></div><div><span>Видео теста</span><h2>{test.video.title}</h2>{test.video.status === "linked" && test.video.url ? <a href={test.video.url}>Смотреть видео</a> : <p>URL ролика не был сохранён в исходной сводке. Блок готов и будет подключён без изменения страницы, когда ссылка будет подтверждена.</p>}</div><span className={styles.videoStatus}>Ожидает ссылку</span></section>
