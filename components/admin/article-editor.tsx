...
function BlockFields({ block, onChange, uploadMedia, uploadProgress }: { block: ArticleBlock; onChange: (next: ArticleBlock) => void; uploadMedia: (file: File, kind: "image" | "video") => Promise<string>; uploadProgress: number | null }) {
  ...
  const richArea = (text: string) => <RichTextEditor value={block.richText} fallback={text} onChange={(richText, plainText) => onChange({ ...block, text: plainText, richText } as ArticleBlock)}/>;
  switch (block.type) {
    case "paragraph": return <div className="admin-field"><span>Текст и оформление</span>{richArea(block.text)}</div>;
    ...
    case "note": return <><label className="admin-field"><span>Заголовок</span>{input(block.title, (title) => onChange({ ...block, title }))}</label><div className="admin-field"><span>Пояснение и оформление</span>{richArea(block.text)}</div></>;
    case "warning": return <><label className="admin-field"><span>Заголовок</span>{input(block.title, (title) => onChange({ ...block, title }))}</label><div className="admin-field"><span>Предупреждение и оформление</span>{richArea(block.text)}</div></>;
    ...
    case "opinion": return <><label className="admin-field"><span>Подпись</span>{input(block.label || "", (label) => onChange({ ...block, label }))}</label><div className="admin-field"><span>Мнение и оформление</span>{richArea(block.text)}</div></>;
    case "video": {
      ...
      return <>
        ...
        <div className="admin-field"><span>Описание и оформление</span>{richArea(block.text)}</div>
