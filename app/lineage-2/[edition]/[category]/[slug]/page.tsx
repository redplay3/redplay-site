...
async function getArticle(params: Params) {
  ...
  const { data } = await supabase.from("articles").select("id,edition,category,slug,title,description,label,cover,highlights,content,published_at,updated_at,seo").eq("edition", params.edition).eq("category", params.category).eq("slug", params.slug).eq("status", "published").maybeSingle();
...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  return {
    title: article.seo?.title || `${article.title} | RedPlay`,
    description: article.seo?.description || article.description,
    alternates: { canonical: `/lineage-2/${article.edition}/${article.category}/${article.slug}` },
    openGraph: article.cover?.src ? { images: [article.cover.src] } : undefined,
  };
}
...
export default async function PublishedArticlePage({ params }: { params: Promise<Params> }) {
  ...
  const playLink = playLinks[article.edition];
  ...
  return <main className="article-page">
    <header className="article-header"><div className="mx-auto flex h-[68px] max-w-[1460px] items-center gap-5 px-4 sm:px-6 lg:px-8">
      ...
      <span className="ml-auto hidden text-sm font-bold text-white/55 md:block">{category} · {edition}</span>
      <a href={playLink} target="_blank" rel="sponsored noopener noreferrer" className="article-play">Играть в {edition} <ArrowUpRight size={15}/></a>
    </div></header>
    ...
    <section className="article-hero">
      ...
      <div className="relative z-10 mx-auto flex min-h-[640px] max-w-[1460px] items-end px-4 pb-12 pt-28 sm:px-6 lg:px-8 lg:pb-16"><div className="max-w-5xl">
        <div className="article-breadcrumb"><Link href="/">Главная</Link><ChevronRight size={14}/><span>{edition}</span><ChevronRight size={14}/><span>{category}</span></div>
    ...
    <div className="mx-auto grid max-w-[1460px] gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[230px_minmax(0,820px)_240px] lg:px-8 lg:py-16">
      ...
      <article className="article-body">
        ...
        <section className="article-next"><div className="article-next-head"><span>Продолжить с RedPlay</span><h2>Выбери следующий шаг</h2><p>Посмотри разборы, получай быстрые новости или начни играть с бонусами.</p></div><div className="article-next-grid"><a className="next-youtube" href="https://www.youtube.com/@iRedP" target="_blank" rel="noopener noreferrer"><Youtube size={24}/><span><small>YouTube</small><strong>Видео и живые разборы</strong></span><ArrowUpRight size={17}/></a><a className="next-telegram" href="https://t.me/redplay2022" target="_blank" rel="noopener noreferrer"><Send size={24}/><span><small>Telegram</small><strong>Новости, тесты и обсуждение</strong></span><ArrowUpRight size={17}/></a><a className="next-game" href={playLink} target="_blank" rel="sponsored noopener noreferrer"><img src={fallbackCovers[article.edition]} alt=""/><span className="next-game-shade"/><span><small>Lineage 2 {edition}</small><strong>Начать играть с бонусами</strong></span><ArrowUpRight size={17}/></a></div></section>
      </article>
      <aside className="article-side"><div className="article-side-card"><span>Lineage 2 {edition}</span><strong>Начни с бонусом RedPlay</strong><p>Бонус для новых и вернувшихся игроков.</p><a href={playLink} target="_blank" rel="sponsored noopener noreferrer">Играть <ArrowUpRight size={15}/></a></div><a className="article-social" href="https://www.youtube.com/@iRedP" target="_blank" rel="noopener noreferrer">Разборы на YouTube <ArrowUpRight size={15}/></a><a className="article-social" href="https://t.me/redplay2022" target="_blank" rel="noopener noreferrer">Новости в Telegram <ArrowUpRight size={15}/></a></aside>
    </div>
