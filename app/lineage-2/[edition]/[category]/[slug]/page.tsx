  ...
  return <main className="article-page">
    ...
    <section className="article-hero">
      ...
      <div className="relative z-10 mx-auto flex min-h-[640px] max-w-[1460px] items-end px-4 pb-12 pt-28 sm:px-6 lg:px-8 lg:pb-16"><div className="max-w-5xl">
        ...
        <div className="mt-7 flex flex-wrap items-center gap-3"><span className="article-label">{article.label || category}</span><span className="article-meta"><CalendarDays size={14}/> {formatDate(article.published_at)}</span><span className="article-meta"><Clock3 size={14}/> {readingTime(sections)} минут</span><ArticleViewCount pageKey={canonicalPath} initialViews={initialViews}/></div>
    ...
    <div className="mx-auto grid max-w-[1460px] gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[230px_minmax(0,820px)_240px] lg:px-8 lg:py-16">
      ...
      <article className="article-body">
        ...
        {article.edition === "main" ? sections.map((section, index) => <section id={section.id} key={section.id}>{index > 0 && <div className="article-block-heading"><span className="article-section-number">{String(index).padStart(2, "0")}</span><h2>{section.label}</h2></div>}<ArticleBlockRenderer blocks={section.blocks}/></section>) : <ArticleAudienceContent sections={sections} targets={audienceTargets}/>} 
        <ArticleSharePanel title={article.title}/>
        <section className="article-next"><div className="article-next-head"><span>Продолжить с RedPlay</span><h2>Выбери следующий шаг</h2><p>Открой другие материалы раздела, посмотри разборы или получай быстрые новости.</p></div><div className="article-next-grid"><Link className="next-section" href={categoryPath}><Bell size={24}/><span><small>{edition}</small><strong>Все материалы: {category}</strong></span><ArrowUpRight size={17}/></Link><a className="next-youtube" href="https://www.youtube.com/@iRedP" target="_blank" rel="noopener noreferrer"><Youtube size={24}/><span><small>YouTube</small><strong>Видео и живые разборы</strong></span><ArrowUpRight size={17}/></a><a className="next-telegram" href="https://t.me/redplay2022" target="_blank" rel="noopener noreferrer"><Send size={24}/><span><small>Telegram</small><strong>Новости, тесты и обсуждение</strong></span><ArrowUpRight size={17}/></a>{articleTargets.map((target) => <a key={target} className="next-game" href={playLinks[target]} target="_blank" rel="sponsored noopener noreferrer"><img src={fallbackCovers[target]} alt=""/><span className="next-game-shade"/><span><small>Lineage 2 {target === "main" ? "Main" : target === "essence" ? "Essence" : "Special Project"}</small><strong>Начать играть с бонусами</strong></span><ArrowUpRight size={17}/></a>)}</div></section>
      </article>
