  ...
  return <main className="min-h-screen overflow-hidden bg-background text-foreground">
    ...
    <section id="top" className="hero-stage">
      ...
      <div className="relative z-10 mx-auto flex min-h-[650px] max-w-[1500px] items-end px-4 pb-8 pt-12 sm:px-6 lg:items-center lg:px-8 lg:py-16">
        <div className="w-full lg:max-w-[62%]">
          ...
          <h1 className="hero-title mt-4">{slide.title}</h1>
    ...
    <section id="updates" className="portal-section"><div className="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
      ...
      <div className="news-layout mt-7">
        <article className="feature-story"><div className="story-art"><img src="/game-main.webp" alt=""/><div className="story-overlay"/></div><div className="relative z-10 flex h-full flex-col justify-end p-6 sm:p-8"><div className="flex items-center gap-3 text-[11px] font-black uppercase tracking-[.13em] text-white/55"><span className="story-badge">Перевод из Кореи</span><span>{featuredUpdate.date}</span><span>• {featuredUpdate.readTime}</span></div><h3>{featuredUpdate.title}</h3><p>{featuredUpdate.summary}</p><div className="mt-5 flex flex-wrap gap-2">{featuredUpdate.tags.map(tag=><span key={tag} className="dark-tag">{tag}</span>)}</div><a href="/publications/main-replica" className="story-link">Читать материал <ArrowUpRight size={17}/></a></div></article>
        <div className="news-stack">{results.map((post,index)=><a key={post.title} href={post.title.startsWith("Replica:") ? "/publications/main-replica" : "#updates"} className={`news-card news-card-${index+1}`}><div className="flex items-center justify-between gap-3"><span className="news-category">{post.category}</span><span className="text-[11px] font-bold text-[#9297a3]">{post.date}</span></div><h3>{post.title}</h3><p>{post.summary}</p><span className="mt-auto flex items-center gap-2 pt-4 text-xs font-black uppercase tracking-[.08em]">Читать <ArrowRight size={14}/></span></a>)}</div>
      </div>
    </div></section>
